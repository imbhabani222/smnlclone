...
const ReportTable = (props: Props) => {
  ...
  const [pageLength, setPageLengthSize] = useState(10);
  const [currentPageNo, setCurrentPage] = useState(1);
  const scrollHeight = showScroll ? `auto` : `calc(60vh - 0px)`;
  const scroll = showScroll ? {} : { y: scrollHeight };
  ...
  const columnData = columnDataSINumber.map((item: any, index) => {
    ...
    return {
      ...
      sortIcon: SortIcon,
      sorter: getSortFunct(newItem?.description, newItem.dataIndex),
            ...
            showScroll
              ...
              ? {
                  ...
                  onChange: (page, pageSize) => {
                    ...
                    totalNumberofRecords && onChangePagination(page, pageSize);
                  },
                }
              : false
          }
          // scroll={dataSourceSiNumberAdded?.length === 10 ? {} : scroll}
