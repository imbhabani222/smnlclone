...
const Create = (props: Props) => {
  ...
  useEffect(() => {
    getFields('Inventory Personal Details And Permissions').then((items: any) => {
    ...
    if (doc_id || id) {
      ...
      getRecordById('inventory_personal_details_and_permissions', data, "general_setup").then(
        (items) => {
          if (items?.name) {
            const da = {
              ...items,
  ...
  const handleFinish = (values: any) => {
    if ((doc_id || id) && formValues?.name) {
      ...
      updateRecord('inventory_personal_details_and_permissions', record, "general_setup").then(
        (items: any) => {
          setmsg((pre) => {
            const message =
              items?.status !== "error"
        ...
        createRecord('inventory_personal_details_and_permissions', record, "general_setup").then(
          (items) => {
            setDoc_id(items?.id);
            setmsg((pre) => {
              const message =
