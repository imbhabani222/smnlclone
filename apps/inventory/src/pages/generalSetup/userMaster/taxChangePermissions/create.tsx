...
const Create = (props: Props) => {
  ...
  useEffect(() => {
    getFields('Inventory Tax Change Permissions').then((items: any) => {
    ...
    if (doc_id || id) {
      ...
      getRecordById('inventory_tax_change_permissions', data, "general_setup").then(
        (items) => {
          if (items?.name) {
            const da = {
              ...items,
  ...
  const handleFinish = (values: any) => {
    if ((doc_id || id) && formValues?.name) {
      ...
      updateRecord('inventory_personal_details_and_permissions', record, "general_setup").then(
        ...
        createRecord('inventory_tax_change_permissions', record, "general_setup").then(
          (items) => {
            setDoc_id(items?.id);
            setmsg((pre) => {
              const message =
