...
const View = (props: any) => {
  ...
  const pageData = {
    pageTitle: 'Complaint Wise Report',
  ...
  useEffect(() => {
    ...
    getSectionWiseReport(
      ...
      console.log(items);
      let columns: any = [];
      const tableKeys = ['Date', 'Complaint', 'Job Card No', 'Approver'];
      ...
      console.log(modifiedData, 'LLLLL');
  ...
  const expandedRowRender = (e: any) => {
    ...
    return (
      <div style={{ margin: '15px', border: '1px solid #d1d1d1' }}>
        <Table column={nestedcolumns} dataSource={e?.part_requests.flat()} />
      </div>
    );
  };
  const onDownloadExcel = () => {
    ...
    handleExportInventory(data, 'Complaint Reports','Complaint Report');
  };
  const onDownloadPdf = () => {
    ...
    const columnd = ['Sl No.', 'Date', 'Complaint', 'Job Card No', 'Approver'];
    rep.html(document.querySelector('#payslip_id'), {
      callback: function (pdf: any) {
        ...
        pdf.save('Complaint Wise Report.pdf');
      },
    });
  };
