...
const View = (props: any) => {
  ...
  const pageData = {
    pageTitle: 'Supplier Purchase Analysis',
  ...
  const reportFunc = (
    ...
    getInventoryReports(
      `supplier_wise_purchase_analysis_report?page=${current_page}&page_length=${page_length}`,
  ...
  useEffect(() => {
    ...
    if (selectedFilter?.financial_year && selectedFilter?.analysis_type) {
      ...
      getInventoryReports(
        `supplier_wise_purchase_analysis_report`,
        'inventory_mis_reports',
        'htsinventory',
        JSON.stringify(selectedFilter)
      ).then((items) => {
