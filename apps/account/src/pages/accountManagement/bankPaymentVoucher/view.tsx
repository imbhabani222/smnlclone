...
const View = (props: Props) => {
  ...
  const tableData = (current_page:number, page_length:number, search:any) => {
    getTableDataWithPagination('bank_payment_voucher', 'account_management', current_page, page_length, 'htsaccount', null, search).then(
      (items: any) => {
  ...
  useEffect(() => {
    setloading(true);
    getDocTypes('Bank Payment Voucher', false, 'htssuite').then((items) => {
      let newData = items.filter((item: any) => {
        const reqFields = [
          // 'bank_ac_ledger_credit',
          'voucher_date',
          // "closing_balance",
          // "upload_doc",
          'action',
    ...
    tableData(paginationData?.current_page, paginationData?.page_length, searchItem);
  }, []);
  ...
  return (
    <div>
      ...
      <Table
        column={columns}
        dataSource={data}
        editUrl="/edit-bank-payment-voucher"
