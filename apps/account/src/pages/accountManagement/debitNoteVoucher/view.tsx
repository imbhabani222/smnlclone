...
const View = (props: Props) => {
  ...
  const tableData = (current_page:number, page_length:number, search:any) => {
    getTableDataWithPagination('debit_note_voucher', 'account_management', current_page, page_length, 'htsaccount', null, search).then(
      (items: any) => {
  ...
  useEffect(() => {
    setloading(true);
    getDocTypes('Debit Note Voucher', false, 'htssuite').then((items) => {
  ...
  return (
    <div>
      ...
      <Table
        column={columns}
        dataSource={data}
        editUrl="/edit-debit-note-voucher"
