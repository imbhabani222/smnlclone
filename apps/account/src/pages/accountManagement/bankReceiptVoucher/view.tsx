...
const View = (props: Props) => {
  ...
  const tableData = (current_page:number, page_length:number, search:any) => {
    getTableDataWithPagination('bank_receipt_voucher', 'account_management', current_page, page_length, 'htsaccount', null, search).then(
      (items: any) => {
  ...
  useEffect(() => {
    setloading(true);
    getDocTypes('Bank Receipt Voucher', false, 'htssuite').then((items:any) => {
    ...
    //     item.bank_ac_ledger = item?.bank_ac_ledger?.bank_ac_ledger;
    //     item.voucher_date = datetoFrom(item.creation);
    //   });
    //   setdata(items);
    //   setloading(false);
    // });
  ...
  return (
    <div>
      ...
      <Table
        column={columns}
        dataSource={data}
        editUrl="/edit-bank-receipt-voucher"
