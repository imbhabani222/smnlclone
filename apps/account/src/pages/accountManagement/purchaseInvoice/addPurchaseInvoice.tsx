import React, { useEffect, useState, useCallback } from 'react';
import DynamicTabs from '../../../../../../libs/common/ui/Tabs/tabs';
import CreatePurchaseInvoice from './createPurchaseInvoice';
import PurchaseInvoiceProduct from './productPurchaseInvoice';
import PurchaseGate from './purchaseGatePass';
import { getSupplierGRN } from 'libs/common/api/doctype';
import { Button } from 'antd';
import {
  useNavigate,
  useSearchParams,
  useParams,
  useLocation,
} from 'react-router-dom';



type Props = {};

const AddDetails = (props: Props) => {
  const [doc_id, setDoc_id] = useState(null);
  const [grn_id, setGrn_id] = useState(null);
  const [supplier_id, setSupplier_id] = useState(null);

  const location = useLocation();
  const navigate = useNavigate();
  const [currentTab, setCurrentTab] = useState<any>();
  const [products, setProducts] = useState<any>([]);
  const [searchParams, setSearchParams] = useSearchParams();
  const id = searchParams.get('id');
  const switchToNextTab = useCallback(
    (key: any, _id: any, grn: any, supplierId: any,productId : any) => {
      if (grn || grn_id) {
        navigate(
          `${key}?id=${doc_id || id || _id}&grn=${grn || grn_id}&supId=${
            supplierId || supplier_id
          }`
        );
      } else {
        navigate(`${key}?id=${doc_id || id || _id}`);
      }
      setCurrentTab(key);
    },
    [doc_id, id, navigate, grn_id]
  );

  


  const tabsData = [
    {
      key: '/create-purchase-invoice',
      label: 'Purchase Invoice',
      children: (
        <CreatePurchaseInvoice
          doc_id={doc_id}
          setDoc_id={setDoc_id}
          setSupplier_id={setSupplier_id}
          setGrn_id={setGrn_id}
          switchToNextTab={switchToNextTab}
        />
      ),
      disabled:
        ((location.pathname === '/add-purchase-invoice-product' && id) || (location.pathname === '/add-invoice-gate-pass' && id)) ? true : false,
    },
    {
      key: '/add-purchase-invoice-product',
      label: 'Product Details',
      children: (
        <PurchaseInvoiceProduct
          doc_id={doc_id}
          setDoc_id={setDoc_id}
          switchToNextTab={switchToNextTab}
          setSupplier_id={setSupplier_id}
          setGrn_id={setGrn_id}
        />
      ),
      disabled:
        ((location.pathname === '/create-purchase-invoice') || (location.pathname === '/add-purchase-invoice-product' )|| (location.pathname === '/add-invoice-gate-pass' && id)) ? true : false,
    },
    {
      key: '/add-invoice-gate-pass',
      label: 'Gate pass',
      children: (
        <PurchaseGate
          doc_id={doc_id}
          setDoc_id={setDoc_id}
          switchToNextTab={switchToNextTab}
        />
      ),
      disabled:
        location.pathname === '/create-purchase-invoice' || (location.pathname === '/add-purchase-invoice-product' ) ? true : false,
    },
  ];
  useEffect(() => {
    if (location.pathname) {
      const tab = tabsData?.find((e: any) => location.pathname.includes(e.key));
      tab && setCurrentTab(tab?.key);
    }
  }, [location.pathname]);
  return (
    <div>
      <DynamicTabs
        items={tabsData}
        currentTab={currentTab}
        onChange={switchToNextTab}
      />
    </div>
  );
};

export default AddDetails;