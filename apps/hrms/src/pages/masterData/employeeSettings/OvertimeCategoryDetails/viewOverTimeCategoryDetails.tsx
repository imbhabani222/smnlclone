...
const ViewOvertimeCategoryDetails = () => {
  ...
  useEffect(() => {
    setloading(true);
    getDocTypes('Over Time Category Details', false, 'htssuite').then((items) =>
      setcolumns(items)
    );
  ...
  const getTableResponse = (current_page: number, page_length: number) => {
    getTableDataWithPagination('over_time_category_details', 'master_data', current_page, page_length, 'htssuite')
      .then((items: any) => {
        setdata(items?.data)
  ...
  return (
    <div>
      ...
      <Table
        column={columns}
        dataSource={data}
        editUrl="/edit-overtime-category"
...
export default ViewOvertimeCategoryDetails;
