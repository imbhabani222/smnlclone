...
const ViewDesignation = () => {
  ...
  useEffect(() => {
    setloading(true);
    getDocTypes('Designation', false, 'htssuite').then((items) =>
      setcolumns(items)
    );
  ...
  const getTableResponse = (current_page:number,page_length:number)=>{
    ...
    getTableDataWithPagination('designation','master_data',current_page,page_length,'htssuite')
    .then((items:any)=>{
      setdata(items?.data)
  ...
  return (
    <div>
      ...
      <Table column={columns} dataSource={data} editUrl="/edit-designation"
