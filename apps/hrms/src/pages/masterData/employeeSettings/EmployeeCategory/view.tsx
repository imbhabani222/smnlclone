...
const View = () => {
  ...
  useEffect(() => {
    setloading(true);
    getDocTypes('Employee Category', false, 'htssuite').then((items) => {
  ...
  const getTableResponse = (current_page: number, page_length: number) => {
    getTableDataWithPagination('employee_category', 'master_data', current_page, page_length, 'htssuite')
      .then((items: any) => {
        setdata(items?.data)
  ...
  return (
    <div>
      ...
      <Table
        column={columns}
        dataSource={data}
        editUrl="/edit-week-off-category"
