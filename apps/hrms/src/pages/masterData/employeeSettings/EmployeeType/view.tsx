...
const ViewEmployeeType = () => {
  ...
  useEffect(() => {
    setloading(true);
    getDocTypes('Employment Type', false, 'htssuite').then((items) =>
      setcolumns(items)
    );
  ...
  const getTableResponse = (current_page:number,page_length:number)=>{
    ...
    getTableDataWithPagination('employment_type','master_data',current_page,page_length,'htssuite')
    .then((items:any)=>{
      setdata(items?.data)
