...
const ViewState = () => {
  ...
  useEffect(() => {
    setloading(true);
    getDocTypes('Shift Master', false, 'htssuite').then((items) => {
  ...
  const getTableResponse = (current_page:number,page_length:number)=>{
    ...
    getTableDataWithPagination('shift_master','master_data',current_page,page_length,'htssuite')
    .then((items:any)=>{
      setdata(items?.data)
  ...
  return (
    <div>
      ...
      <Table column={columns} dataSource={data} editUrl="/edit-shift-master"
