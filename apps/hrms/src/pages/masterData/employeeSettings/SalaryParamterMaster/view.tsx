...
const ViewSalaryParamterMaster = () => {
  ...
  useEffect(() => {
    setloading(true);
    getDocTypes('Salary Parameter Master', false, 'htssuite').then((items) => {
      let newData = items.filter((item: any) => {
        const reqfields = [
          'salary_parameter_name',
          'display_on_payslip',
          'display_on_payregister',
          'addition_deduction',
          'monthly_yearly',
          'type',
          'percentage_value',
  ...
  const getTableResponse = (current_page:number,page_length:number)=>{
    ...
    getTableDataWithPagination('salary_parameter_master','master_data',current_page,page_length,'htssuite')
    .then((items:any)=>{
      setdata(items?.data)
...
export default ViewSalaryParamterMaster;
