...
const ViewClaims = () => {
  ...
  useEffect(() => {
    setloading(true);
    getDocTypes('Nature of Claim', false, 'htssuite').then((items) => {
      setcolumns(items)
      setloading(false)
    }
    );
  ...
  const getTableResponse = (current_page: number, page_length: number) => {
    setloading(true)
    getTableDataWithPagination('nature_of_claim', 'master_data', current_page, page_length, 'htssuite')
      .then((items: any) => {
        setdata(items?.data)
