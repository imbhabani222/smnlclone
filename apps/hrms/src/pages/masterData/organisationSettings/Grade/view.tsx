...
const ViewGrade = () => {
  ...
  useEffect(() => {
    setloading(true);
    getDocTypes('Grade', false, 'htssuite').then((items) => {
      setloading(false)
      setcolumns(items)}
      );
  ...
  const getTableResponse = (current_page: number, page_length: number) => {
    setloading(true)
    getTableDataWithPagination('grade', 'master_data', current_page, page_length, 'htssuite')
      .then((items: any) => {
        setdata(items?.data)
  ...
  return (
    <div>
      ...
      <Table column={columns} dataSource={data} editUrl="/edit-grade" 
